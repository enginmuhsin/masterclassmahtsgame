...
def cevap_ver(index, buton_tipi):
    ...
    if buton_tipi == "sol":
        if dogru_mu: # Ve sayı gerçekten o özelliğe sahipse
            kullanici_bildi_mi = True
            
    # Kullanıcı Sağ Butona (HAYIR/ÇİFT) bastıysa:
    elif buton_tipi == "sag":
        if not dogru_mu: # Ve sayı gerçekten o özelliğe sahip DEĞİLSE
            kullanici_bildi_mi = True
            
    # Sonucu Kaydet ve Puanla
    if kullanici_bildi_mi:
        st.session_state.sorular_cevaplandi[index] = "dogru" # YEŞİL YANACAK
        
        # Puan hesapla (Sol butonsa p_d, sağ butonsa p_y kadar puan ver)
        kazanc = p_d if buton_tipi == "sol" else p_y
        st.session_state.puan += kazanc
        st.toast(f"{random.choice(OVGULER)} +{kazanc} Puan", icon="✅")
    else:
        st.session_state.sorular_cevaplandi[index] = "yanlis" # KIRMIZI YANACAK
...
if secim == "🎮 Oyun Modu":
    ...
    # Session State Başlatma (True/False yerine None kullanıyoruz)
    if 'hedef_sayi' not in st.session_state:
        st.session_state.hedef_sayi = 0
        st.session_state.puan = 0
        st.session_state.gizli = True
        # None: Cevaplanmadı, "dogru": Bildi, "yanlis": Bilemedi
    ...
    if st.session_state.hedef_sayi != 0:
        ...
        for i, (soru, func, p_d, p_y, sol_txt, sag_txt) in enumerate(OZELLIKLER):
            durum = st.session_state.sorular_cevaplandi[i] # None, "dogru" veya "yanlis"
            ...
            # Cevaplanmışsa Sonucu Göster (RENKLİ)
            else:
                # Gerçek cevabı hesapla (Metin oluşturmak için)
                dogru_mu = func(st.session_state.hedef_sayi)
                ...
                # KUTUYU BOYAMA ZAMANI
                if durum == "dogru":
                    # Kullanıcı DOĞRU bilmiş -> YEŞİL
                    st.success(f"✅ DOĞRU! -> **{gercek_cevap_metni}**")
                else:
                    # Kullanıcı YANLIŞ bilmiş -> KIRMIZI
